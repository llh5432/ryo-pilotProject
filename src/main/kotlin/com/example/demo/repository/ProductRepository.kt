...
interface ProductRepository : JpaRepository<Product, Int>{// interface에 미리 JpaReopository메서드를 정의해둔것   <entityClass, pk>
    ...
    fun findByMenuTypeEqualsAndPriceGreaterThanEqual(selectMenuType: MenuType, selectMinPrice: Int): List<Product> // 메뉴타입 + ~원 이상
    fun findByMenuTypeEqualsAndPriceLessThanEqual(selectMenuType: MenuType, selectMaxPrice: Int): List<Product> // 메뉴타입 + ~원 이하
